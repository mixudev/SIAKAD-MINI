...
<div class="animate-fade-in">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-lg font-bold text-slate-900">Kartu Hasil Studi (KHS)</h1>
            <p class="text-sm text-slate-500">Lihat hasil studi Anda per semester.</p>
        </div>
    </div>

    <div class="bg-white border border-slate-200 mb-6">
        <div class="px-4 py-3 border-b border-slate-100">
            <form method="GET" action="{{ route('mahasiswa.khs.index') }}">
                <div class="flex gap-3 items-center">
                    ...
                    <select name="semester_id" onchange="this.form.submit()" class="px-3 py-2 border border-slate-200 text-sm focus:outline-none focus:border-slate-900">
                        @foreach($semesterList as $s)
                            <option value="{{ $s->id }}" {{ $selectedSemester && $s->id == $selectedSemester->id ? 'selected' : '' }}>
                                {{ $s->nama }}
                            </option>
                        @endforeach
        ...
        @if ($selectedSemester && $daftarNilai->isNotEmpty())
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Bobot Mutu</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold {{ $nilai->nilai_huruf && $nilai->nilai_huruf[0] >= 'C' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
                                {{ $nilai->nilai_huruf ?? '-' }}
                            </span>
                        </td>
        ...
        <div class="px-4 py-4 border-t border-slate-100 bg-slate-50">
            <div class="flex items-center justify-end gap-8">
                ...
                <div class="text-right">
                    ...
                    <p class="text-2xl font-bold text-slate-900">{{ number_format($ipk, 2) }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border border-slate-200">
        <div class="px-4 py-3 border-b border-slate-100">
            <h2 class="text-sm font-bold text-slate-900">Transkrip Akademik</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                ...
                <tfoot class="bg-slate-50 border-t border-slate-100">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-sm font-bold text-slate-900">{{ number_format($ipk, 2) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
